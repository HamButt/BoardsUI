...
function Giphy({gifSearchValue,gifData,setGif,setGIFComponent,setGifSection,getGifsFromGiphy,isFetchingGIfs}) {
    ...
    <div style={{maxHeight:"330px"}} className="gifs mx-10 mt-4 flex-wrap overflow-auto flex items-start justify-evenly">
        ...
        { gifSearchValue && gifData.length ?  
            gifData.map((gif, index) => { 
                ...
                { 
                    gifData.length ? <motion.button disabled={isFetchingGIfs ? true : false} whileTap={{ scale: 0.9 }}  onClick={getGifsFromGiphy} 
                    className='mt-2 border text-sm my-1 px-2 py-1 rounded-md border-black text-black'>{isFetchingGIfs ? "Loading..." : "Load more"}</motion.button> 
                : ""
            }
        </div>
  )
